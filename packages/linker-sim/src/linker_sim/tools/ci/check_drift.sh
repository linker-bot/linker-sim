...
#
# Usage:
#   bash packages/linker-sim/src/linker_sim/tools/ci/check_drift.sh              # check all workstations
#   bash packages/linker-sim/src/linker_sim/tools/ci/check_drift.sh <ws_name>    # check one workstation
#
...
if [[ ${fail} -ne 0 ]]; then
    ...
    echo "  for ws in assets/workstations/*/; do python -m linker_sim.tools.composer.compose \"\$ws\"; done" >&2
